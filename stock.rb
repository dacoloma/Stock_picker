def trader_du_dimanche(lst)
    marge=[]
    indice=[]
    for a in (0...lst.length) do
        v = a+1
        while v < lst.length
            marge << lst[v] - lst[a]
            indice << a.to_s + "," + v.to_s
            v += 1
        end
    end
    #puts marge.max
    return indice[marge.index(marge.max)]
end


def trader_du_lundi(lst_hash)
    marge={}
    indice={}
    entreprise={}
    for a in lst_hash

        a.each do |key,value|
            entreprise[key] = []
        end
    end

    for a in lst_hash
        a.each do |key,value|
            entreprise[key] << value
        end
        #entreprise[key] = val
        #entreprise.each do |key,value|
    end



    entreprise.each  { |key,value|
        puts "#{key} : #{trader_du_dimanche(value)} "
    }

        #puts "#{k} : #{v}"
end




#test = [17,3,6,9,15,8,6,1,10]
#trader_du_dimanche(test)

jour_1 = { :GOO => 15, :MMM => 14, :ADBE => 12, :EA=> 13, :BA => 8, :KO => 10, :XOM => 20, :GPS => 7, :MCD => 11, :DIS => 15, :CRM => 6, :JNJ => 10 }
jour_2 = { :GOO => 8, :MMM => 20, :ADBE => 3, :EA=> 10, :BA => 5, :KO => 19, :XOM => 12, :GPS => 6, :MCD => 15, :DIS => 9, :CRM => 10, :JNJ => 17 }
jour_3 = { :GOO => 3, :MMM => 8, :ADBE => 15, :EA=> 5, :BA => 10, :KO => 5, :XOM => 15, :GPS => 13, :MCD => 10, :DIS => 18, :CRM => 5, :JNJ => 14 }
jour_4 = { :GOO => 17, :MMM => 3, :ADBE => 6, :EA=> 9, :BA => 15, :KO => 6, :XOM => 8, :GPS => 1, :MCD => 10, :DIS => 15, :CRM => 18, :JNJ => 3 }
jour_5 = { :GOO => 8, :MMM => 18, :ADBE => 4, :EA=> 6, :BA => 15, :KO => 18, :XOM => 3, :GPS => 12, :MCD => 19, :DIS => 3, :CRM => 7, :JNJ => 9 }
jour_6 = { :GOO => 10, :MMM => 12, :ADBE => 8, :EA=> 3, :BA => 18, :KO => 20, :XOM => 5, :GPS => 11, :MCD => 3, :DIS => 9, :CRM => 8, :JNJ => 15 }
jour_7 = { :GOO => 17, :MMM => 14, :ADBE => 2, :EA=> 17, :BA => 7, :KO => 13, :XOM => 1, :GPS => 15, :MCD => 15, :DIS => 10, :CRM => 9, :JNJ => 17 }

test_lvlup = [jour_1,jour_2,jour_3,jour_4,jour_5,jour_6,jour_7]
trader_du_lundi(test_lvlup)

=begin
for a in (0...lst.length) do
    v = a+1
    while v < lst.length


        marge << lst[v] - lst[a]
        indice << a.to_s + "," + v.to_s
        v += 1
    end
end
#puts marge.max
puts indice[marge.index(marge.max)]
=end
